...
namespace MeltBot.Modules
{
    ...
    internal class AddCommands : BaseCommandModule
    {
        ...
        public async Task Update(CommandContext ctx)
        {
            ...
            {
                var t = Context.CraftEssences.ToList();
                for (int i = 0; i < t.Count; i++)
                {
                    using(var c = new RunsContext())
                    {
                        t[i] = await CeData(t[i].Id, t[i]);
                        c.SaveChanges();
                    }
                }
                var s = Context.Servants.ToList();
                for (int i = 0; i < t.Count; i++)
                {
                    using (var c = new RunsContext())
                    {
                        s[i] = await ServantData(s[i].Id, s[i]);
                        c.SaveChanges();
                    }
                }
        ...
        private static async Task<Servant> ServantData(int servantId, Servant? s)
        {
            using (var client = new HttpClient())
            {
                ...
                if (j is null) throw new CustomException("Problem with " + servantId);
                ...
                s.BaseMaxLvl = j.Value<short>("lvMax");
            ...
            [Description("New nickname")] string nickname)
        {
            ...
            try
            {
                if (Context.ServantAliases.Any(x => x.Nickname == nickname)) throw new CustomException($"nickname {nickname} already exists");
                Servant? s;
                if (int.TryParse(servant, out int id))
                {
                    s = Context.Servants.FirstOrDefault(s => s.Id == id || s.CollectionNo == id);
                    if (s is null) throw new CustomException($"{servant} could not be found");
                }
                else
                {
        ...
        private static async Task<Quest> QuestData(int questId, Quest? q)
        {
            using (var client = new HttpClient())
            {
                ...
                if (j is null) throw new CustomException("Problem with " + questId);
                string n = j.Value<string>("name");
            ...
            [Description("New nickname")] string nickname)
        {
            ...
            try
            {
                if (Context.QuestAliases.Any(x => x.Nickname == nickname)) throw new CustomException($"nickname {nickname} already exists");
                Quest? q;
        ...
        private static async Task<CraftEssence> CeData(int ceId, CraftEssence? ce)
        {
            using (var client = new HttpClient())
            {
                ...
                if (j is null) throw new CustomException("Problem with " + ceId);
                string n = j.Value<string>("name");
                ...
                ce.BaseMaxLvl = j.Value<short>("lvMax");
            ...
            [Description("New nickname")] string nickname)
        {
            ...
            try
            {
                if (Context.CraftEssenceAliases.Any(x => x.Nickname == nickname)) throw new CustomException($"nickname {nickname} already exists");
                CraftEssence? c;
                if (int.TryParse(ce, out int id))
                {
                    c = Context.CraftEssences.FirstOrDefault(q => q.Id == id);
                    if (c is null) throw new CustomException($"{ce} could not be found");
                }
                else
                {
        ...
        private static async Task<MysticCode> McData(int mcId, MysticCode? mc)
        {
            using (var client = new HttpClient())
            {
                ...
                if (j is null) throw new CustomException("Problem with " + mcId);
                string n = j.Value<string>("name");
            ...
            [Description("New nickname")] string nickname)
        {
            ...
            try
            {
                if (Context.MysticCodeAliases.Any(x => x.Nickname == nickname)) throw new CustomException($"nickname {nickname} already exists");
                MysticCode? m;
                if (int.TryParse(mc, out int id))
                {
                    m = Context.MysticCodes.FirstOrDefault(q => q.Id == id);
                    if (m is null) throw new CustomException($"{mc} could not be found");
                }
                else
                {
